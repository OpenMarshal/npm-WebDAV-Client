        ...
        this.request({
            ...
            try {
                ...
                var results = xml_js_builder_1.XML.parse(body)
                    ...
                    .map(function (el) {
                    var href = el.find('DAV:href').findText(), hrefWithoutTrailingSlash = (href.lastIndexOf('/') === href.length - 1 ? href.slice(0, -1) : href), fullPath = decodeURI(Url.parse(href).pathname), name = Path.basename(fullPath);
